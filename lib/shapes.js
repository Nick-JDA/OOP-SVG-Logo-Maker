...
class Shapes {
    ...
    // Create some methods to set the values of text, textColor, and shapeColor
    setText(text) {
        this.text = text;
    }
...
example.setText('SVG');
example.setTextColor('white');
example.setShapeColor('green');
...
class Triangle extends Shapes {
  ...
  <polygon points="150,0 300,300 0,300" fill="${this.shapeColor}" />
   <text x="150" y="125" font-weight="bold" font-size="50"text-anchor="middle"fill="${this.textColor}"dy="0.35em">${this.text}</text>
  Sorry, your browser does not support inline SVG.
</svg>`
    }
}
